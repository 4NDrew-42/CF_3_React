...
export const ProfileView = ({ user, movies, token, onFavoriteToggle }) => {
	...
	useEffect(() => {
		...
		const fetchUserData = async () => {
			try {
				const res = await fetch(`http://54.162.113.255/users/${user.username}`, {
	...
	const handleUpdate = async (event) => {
		...
		try {
			const response = await fetch(`http://54.162.113.255/users/${user.username}`, {
				method: 'PUT',
				headers: {
	...
	const handleFavoriteToggle = async (movieID) => {
		try {
			...
			const response = await fetch(`http://54.162.113.255/users/${user.username}/movies/${movieID}`, {
				method,
				headers: {
					Authorization: `Bearer ${token}`,
				},
