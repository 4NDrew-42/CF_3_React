...
import ImageUpload from '../image-upload/image-upload.jsx';
...
export const MainView = () => {
  ...
  const fetchUserData = () => {
    ...
    fetch(`http://35.173.126.252/users/${user.username}`, {
      headers: {
  ...
  useEffect(() => {
    ...
    fetch('http://35.173.126.252/movies', {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  const handleFavoriteToggle = async (movieId) => {
    ...
    try {
      const response = await fetch(
        `http://35.173.126.252/users/${user.username}/movies/${movieId}`,
        {
          method,
          headers: {
            Authorization: `Bearer ${token}`,
