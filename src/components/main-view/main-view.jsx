...
export const MainView = () => {
	...
	useEffect(() => {
		...
		fetch('http://54.162.113.255/movies', {
			headers: {
	...
	useEffect(() => {
		...
		fetch(`http://54.162.113.255/movies/users/${user.username}`, {
			headers: {
				Authorization: `Bearer ${token}`,
			},
		})
	...
	const handleFavoriteToggle = async (movieId) => {
		...
		try {
			const response = await fetch(`http://54.162.113.255/movies/users/${user.username}/movies/${movieId}`, {
				method,
				headers: {
					Authorization: `Bearer ${token}`,
				},
